...
class HybridIDS:
    ...
    Tích hợp Luồng 1 (Supervised - SVM) và Luồng 2 (Unsupervised - K-Means)
    """
    def __init__(self, sup_model_path, anomaly_model_path):
        ...
        # Tải mô hình Dị thường từ TV 4 (K-Means)
...
class Evaluator:
    ...
    def evaluate_and_compare(y_true, y_pred_sup, y_pred_integrated):
        ...
        fpr_sup = fp1 / (fp1 + tn1) if (fp1 + tn1) > 0 else 0 # Tỷ lệ báo nhầm
        ...
        print(f"\n2. INTEGRATED SYSTEM (SVM + K-Means):")
        ...
        print(f"     => TRADE-OFF: False Positive Rate increased by {(fpr_int - fpr_sup):.2%} compared to baseline Ehrenberg.")
        print("="*40)
...
if __name__ == "__main__":
    ...
    # NẠP MÔ HÌNH: SVM + K-Means
    ids = HybridIDS('supervised_svm_model.pkl', 'unsupervised_kmeans_model.pkl')
    
    print("\nRunning Scenario 1: SVM Only...")
    preds_sup = ids.predict_sup_only(X_test_sup)
    
    print("Running Scenario 2: 2-Layer Integrated System (SVM + K-Means)...")
