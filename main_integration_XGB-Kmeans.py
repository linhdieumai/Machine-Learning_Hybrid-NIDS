...
class HybridIDS:
    ...
    Tích hợp Luồng 1 (Supervised - XGBoost) và Luồng 2 (Unsupervised - Anomaly)
    """
    ...
    def predict_tree_only(self, X):
        """Kịch bản 1: Chỉ đi qua hàng rào lớp 1 (XGBoost)"""
    ...
    def predict_integrated(self, X_sup, X_unsup):
        ...
        Sử dụng logic tính khoảng cách của thuật toán K-Means
        """
        # Bước 1: Đi qua mô hình Học có giám sát (RF hoặc XGBoost)
        ...
        if len(normal_indices) > 0:
            ...
            # --- LOGIC CHO K-MEANS ---
            # Tính khoảng cách từ các gói tin đến tâm cụm Normal (Cột 0)
            distances = self.anomaly_model.transform(X_normal_suspects)[:, 0]
            
            # Nếu khoảng cách > Threshold => Đánh dấu là Dị thường (1)
            anomalies_mask = (distances > self.threshold).astype(int)
            
            # Cập nhật đè kết quả tấn công lên nhãn Normal cũ
...
class Evaluator:
    ...
    def evaluate_and_compare(y_true, y_pred_tree, y_pred_integrated):
        ...
        print("="*40)
        print(f"1. PIPELINE 1 ONLY (XGBoost):")
    ...
    def plot_tradeoff(fn_tree, fn_int, fp_tree, fp_int):
        ...
        rects1 = ax.bar(x - width/2, tree_scores, width, label='XGBoost Only', color='#3498db')
...
if __name__ == "__main__":
    ...
    # NẠP MÔ HÌNH: XGBoost + K-Means
    ids = HybridIDS('supervised_xgb_model.pkl', 'unsupervised_kmeans_model.pkl')
    
    print("\nRunning Scenario 1: XGBoost Only...")
    preds_tree = ids.predict_tree_only(X_test_sup)
    
    print("Running Scenario 2: 2-Layer Integrated System (XGBoost + K-Means)...")
